...
For example, to set the `mongo` service image, rather than editing `lib/docker-compose.mongo.yml` directly, set the image using the environment variable `MONGO_IMAGE` in `config/overleaf.rc`. For more information see the [Toolkit settings](../../configuration/overleaf-toolkit/toolkit-settings.md) page.
{% endhint %}
