...
To avoid manual updates with `bin/up` your instance everytime, you can automate updates to your TeX Live image using the following script. Add it as a cron job, making sure the cron user has permission to run `docker`. Run `crontab -e` to add the following line to the end.
